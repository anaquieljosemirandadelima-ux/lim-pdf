...
async function buildBudget() {
  ...
  console.log(JSON.stringify({ ok: true, suite: "performance-build", jsTotalMb: Number((jsTotal / 1024 / 1024).toFixed(2)), cssTotalMb: Number((cssTotal / 1024 / 1024).toFixed(2)), largestJsKb: Math.round(largestJs.size / 1024), largestCssKb: Math.round(largestCss.size / 1024) }));
}
  ...
  try {
    for (const route of routes) {
      ...
      const metrics = await page.evaluate(() => {
        ...
        return { resourceCount: resources.length, jsCssTransfer: jsCss.reduce((sum, entry) => sum + (entry.transferSize || entry.encodedBodySize || 0), 0), maxResource: resources.reduce((max, entry) => Math.max(max, entry.transferSize || entry.encodedBodySize || 0), 0) };
      });
      assert.ok(elapsed < 12_000, `${route}: conteúdo utilizável local acima de 12s (${elapsed}ms)`);
