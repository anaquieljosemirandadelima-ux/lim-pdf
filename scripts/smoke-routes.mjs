...
for (const route of routes) {
  ...
  if (route === "/ferramentas" && (!text.includes("Todas as ferramentas") || !text.includes("Converter PDF") || !text.includes("OCR PDF"))) throw new Error("catalog missing priority actions");
  if (route === "/ferramentas/editar-pdf") for (const marker of ["Studio", "Modo preciso", "Página", "Documento", "application/ld+json"]) if (!text.includes(marker)) throw new Error(`editor route missing ${marker}`);
  ...
  if (route === "/sitemap.xml") for (const marker of ["/ferramentas/converter-pdf", "/ferramentas/ocr-pdf", "/ferramentas/dimensionar-pdf", "/sobre", "/guias/"]) if (!text.includes(marker)) throw new Error(`sitemap missing ${marker}`);
}

for (const route of ["/faq", "/api/colorcopia-guia", "/api/contato"]) {
  const response = await fetch(`${base}${route}`, { redirect: "manual" });
  if (response.status !== 404 && route !== "/api/contato") throw new Error(`${route} should return 404, got ${response.status}`);
  if (route === "/api/contato" && response.status !== 404 && response.status !== 405) throw new Error(`${route} should be removed, got ${response.status}`);
}
...
console.log(JSON.stringify({ ok: true, suite: "routes", checked: routes.length, removed: 3, telemetry: true, newUx: true, seoContent: true }));
