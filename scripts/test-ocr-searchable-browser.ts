...
async function main() {
  ...
  try {
    ...
    await page.locator('.pro-pdf-workspace .drop-zone input[type="file"]').setInputFiles(join(fixtureDir, "one-page.pdf"));
    ...
    assert.ok(normalized.length >= 3, `OCR deveria gerar texto pesquisável; texto extraído: ${JSON.stringify(normalized)}`);
    console.log(JSON.stringify({ ok: true, suite: "ocr-searchable-browser", pages: document.numPages, extractedCharacters: normalized.length, searchableLayer: true }));
