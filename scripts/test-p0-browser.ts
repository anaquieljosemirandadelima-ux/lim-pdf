...
async function runPrintCenter(page: Page) {
  await navigate(page, "/ferramentas/criar-livreto-pdf");
  assert.match(await page.title(), /Centro de impressão/i);
  assert.equal(await page.getByText("Processamento local", { exact: false }).count() > 0, true, "Centro de Impressão deve informar processamento local");
  ...
  try {
    ...
    throw new Error(`Centro de Impressão não liberou as configurações após upload. Input: ${JSON.stringify(fileState)}. Estado: ${visible.slice(0, 1200)}`, { cause: error });
  }
  ...
  assert.equal(await selects.count(), 2, "Centro de Impressão deve começar com finalidade e papel; a margem é numérica");
  ...
  try {
    await page.getByText("PDF pronto. Revise o plano e abra a saída para imprimir no computador.", { exact: true }).waitFor({ state: "visible", timeout: 30_000 });
  } catch (error) {
    const workspace = await page.locator(".print-center-workspace").innerText().catch(() => "");
    throw new Error(`Saída do Centro de Impressão apareceu, mas a mensagem final não foi encontrada. Estado: ${workspace.slice(-1800)}`, { cause: error });
  }
  ...
  const pages = await assertPdf(output, "Centro de Impressão");
  assert.ok(pages >= 1, "Centro de Impressão deve produzir pelo menos uma página");
}
...
async function runPreflight(page: Page) {
  ...
  const [download] = await Promise.all([
    page.waitForEvent("download", { timeout: 30_000 }),
    page.getByRole("button", { name: /Criar cópia sanitizada/i }).click(),
  ]);
  const output = join(downloadDir, download.suggestedFilename());
  await download.saveAs(output);
...
async function main() {
  ...
  try {
    ...
    console.log(JSON.stringify({ ok: true, suite: "p0-browser", printCenter: true, localQueue: true, privacyInspector: true }));
  } finally {
