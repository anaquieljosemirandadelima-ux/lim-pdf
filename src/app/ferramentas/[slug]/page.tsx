...
import { ArrowRight, CheckCircle2, ImagePlus, Info, LockKeyhole, ShieldCheck, Type, UploadCloud } from "lucide-react";
...
export default async function ToolPage({ params }: ToolPageProps) {
        ...
        <section className="editor-page-heading editor-page-heading-v3">
          <div className="container">
            ...
            <div className="editor-heading-actions">
              ...
              <Link className="secondary-button" href="/ferramentas/adicionar-imagem-pdf"><ImagePlus size={17} /> Adicionar imagem</Link>
              <a className="primary-button" href="#editor-workspace"><UploadCloud size={17} /> Selecionar PDF</a>
            </div>
          </div>
        </section>
  ...
  return (
    <>
      ...
      <section className="tool-seo-summary"><div className="container tool-seo-summary-grid"><article><span className="eyebrow">Sobre a ferramenta</span><h2>{tool.name} sem instalar programa</h2><p>{tool.intro}</p><ul>{tool.useCases.map((item) => <li key={item}><CheckCircle2 size={16} /> {item}</li>)}</ul></article><aside><Info size={22} /><h2>Antes de processar</h2>{tool.limitations.map((item) => <p key={item}>{item}</p>)}</aside></div></section>
