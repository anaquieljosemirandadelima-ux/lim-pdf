...
export function OcrWorkspace() {
  ...
  return <section className={`workspace ocr-workspace ${processing ? "is-processing" : ""}`}>
    ...
    <button className="process-button prominent-process" type="button" disabled={!files.length || processing} onClick={() => void process()}>{processing ? <><LoaderCircle className="spin" size={18} /> {status.message || "Reconhecendo…"}</> : <><Search size={18} /> {files.length > 1 ? "Reconhecer e baixar ZIP" : "Reconhecer texto e baixar PDF"}</>}</button>
