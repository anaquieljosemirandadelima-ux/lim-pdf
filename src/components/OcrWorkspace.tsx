...
export function OcrWorkspace() {
  ...
  function selectFile(selected: File | null) {
    ...
    if (selected.size > MAX_FILE_SIZE) {
      setStatus({ type: "error", message: "O PDF ultrapassa 60 MB." });
      return;
    }
    setFile(selected);
    setReport(null);
  ...
  async function process() {
    ...
    try {
      const result = await createSearchablePdf(file, languages, (message, progress) => setStatus({ type: "processing", message, progress }));
      ...
      setStatus({ type: "error", message: error instanceof Error ? error.message : "Não foi possível executar o OCR." });
    }
  }

  return (
    <section className="workspace ocr-workspace">
      ...
      <div className="tool-options ocr-options">
        ...
        <div className="ocr-info-card"><Search size={17} /><span><strong>Saída pesquisável</strong><small>O PDF mantém a página visual e recebe texto invisível alinhado ao reconhecimento.</small></span></div>
        <div className="ocr-info-card secure"><ShieldCheck size={17} /><span><strong>Privacidade</strong><small>O documento não é enviado ao LIM PDF. O navegador baixa o motor OCR e os modelos de idioma necessários.</small></span></div>
      </div>

      <button className="process-button prominent-process" type="button" disabled={!file || status.type === "processing"} onClick={() => void process()}>{status.type === "processing" ? <><LoaderCircle className="spin" size={18} /> {status.message || "Reconhecendo…"}</> : <><Search size={18} /> Reconhecer texto e baixar PDF</>}</button>
      {status.type === "processing" && typeof status.progress === "number" ? <div className="ocr-progress"><i style={{ width: `${Math.max(2, Math.min(100, status.progress))}%` }} /><span>{Math.round(status.progress)}%</span></div> : null}
