...
export function OcrWorkspace() {
  ...
  return <section className={`workspace ocr-workspace ${processing ? "is-processing" : ""}`}>
    <div className="drop-zone" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); if (!processing) selectFile(event.dataTransfer.files[0] || null); }}>
      ...
      <input ref={inputRef} hidden type="file" accept="application/pdf,.pdf" disabled={processing} onClick={(event) => { event.currentTarget.value = ""; }} onChange={(event) => selectFile(event.target.files?.[0] || null)} /><small>Até {formatFileSizeLimit()} · até 80 páginas por execução · OCR no navegador</small>
    </div>
    ...
    {file && getFileSizeGuidance(file).tier !== "standard" ? <div className="large-file-notice" role="status"><ShieldCheck size={16} /><span><strong>Processamento de arquivo grande</strong><small>{getFileSizeGuidance(file).message} O OCR pode demorar mais porque analisa cada página localmente.</small></span></div> : null}
