...
export function ProPdfWorkspace({ tool }: { tool: ProToolDefinition }) {
  ...
  const [cleanDeskew, setCleanDeskew] = useState(true); const [cleanBlank, setCleanBlank] = useState(true); const [cleanStrength, setCleanStrength] = useState(1); const [cleanRotate, setCleanRotate] = useState<ScanRotation>(0);
  ...
  return <section className="workspace pro-pdf-workspace">
    <div className="drop-zone" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); void selectFiles(event.dataTransfer.files); }}>
      ...
      <input ref={fileInputRef} type="file" accept={tool.accept} multiple={multiple} hidden onChange={(event) => void selectFiles(event.target.files)} /><small>Até 80 MB por arquivo · processamento local</small>
    </div>
    ...
    <div className="tool-options pro-tool-options">
      ...
      {tool.slug === "limpar-documento-digitalizado" ? <><label className="checkbox-line"><input type="checkbox" checked={cleanDeskew} onChange={(event) => setCleanDeskew(event.target.checked)} /> Endireitar automaticamente</label><label className="checkbox-line"><input type="checkbox" checked={cleanBlank} onChange={(event) => setCleanBlank(event.target.checked)} /> Remover páginas em branco</label><label><span>Força da limpeza</span><input type="range" min={0} max={2} step={.25} value={cleanStrength} onChange={(event) => setCleanStrength(Number(event.target.value))} /></label><label><span>Rotação</span><select value={cleanRotate} onChange={(event) => setCleanRotate(Number(event.target.value) as ScanRotation)}><option value={0}>Automática/nenhuma</option><option value={90}>90°</option><option value={180}>180°</option><option value={270}>270°</option></select></label></> : null}
      ...
      {tool.slug === "assinatura-digital-pdf" ? <><div className="pro-option-grid two"><button className="secondary-button" type="button" onClick={() => certificateRef.current?.click()}><ShieldCheck size={16} /> {certificateName || "Certificado X.509 PEM"}</button><button className="secondary-button" type="button" onClick={() => privateKeyRef.current?.click()}><ShieldCheck size={16} /> {privateKeyName || "Chave RSA PKCS#8 PEM"}</button><input ref={certificateRef} hidden type="file" accept=".pem,.crt,.cer,text/plain" onChange={(event) => void readTextFile(event.target.files, "certificate")} /><input ref={privateKeyRef} hidden type="file" accept=".pem,.key,text/plain" onChange={(event) => void readTextFile(event.target.files, "key")} /></div><label><span>Nome do signatário</span><input value={signerName} onChange={(event) => setSignerName(event.target.value)} /></label><label><span>Motivo</span><input value={signReason} onChange={(event) => setSignReason(event.target.value)} /></label><label className="checkbox-line"><input type="checkbox" checked={signVisible} onChange={(event) => setSignVisible(event.target.checked)} /> Inserir aparência visual da assinatura</label></> : null}
    </div>

    <button className="process-button" type="button" disabled={!ready || status.type === "processing"} onClick={() => void process()}>{status.type === "processing" ? <><LoaderCircle className="spin" size={18} /> {status.message || "Processando…"}</> : <>Executar {tool.name}</>}</button>
