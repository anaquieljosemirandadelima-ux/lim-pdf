  ...
  async function process() {
    ...
    try {
      ...
      if (tool.slug === "comparar-pdfs") {
        const result = await comparePdfs(files[0], files[1], compareThreshold, progress); downloadBytes(result.bytes, result.filename); setReport(result.report);
