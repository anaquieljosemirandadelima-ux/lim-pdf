...
type CatalogTool = AnyToolDefinition | ProToolDefinition;
...
function toolBySlug(slug: CatalogSlug): CatalogTool | undefined { return proToolBySlug.get(slug as ProToolSlug) || allToolBySlug.get(slug as AllToolSlug); }
...
function parseStoredToolSlugs(raw: string): CatalogSlug[] {
  try { const parsed = JSON.parse(raw) as unknown; return Array.isArray(parsed) ? parsed.filter((value): value is CatalogSlug => typeof value === "string" && Boolean(toolBySlug(value as CatalogSlug))) : []; } catch { return []; }
}
...
function ToolItem({ tool, favorite, onFavorite }: { tool: CatalogTool; favorite: boolean; onFavorite: (slug: CatalogSlug) => void }) {
  ...
  return <div className={`reference-catalog-tool-wrap ${favorite ? "favorite" : ""}`}>
    <Link href={`/ferramentas/${slug}`} className="reference-catalog-tool" onClick={() => recordRecentTool(slug as unknown as AllToolSlug)}>
      ...
      <span className="reference-catalog-copy"><strong>{tool.name}</strong><small>{tool.shortDescription}</small></span><ArrowRight size={17} />
    </Link>
    <button className="reference-favorite-button" type="button" aria-label={favorite ? `Remover ${tool.name} dos favoritos` : `Adicionar ${tool.name} aos favoritos`} aria-pressed={favorite} title={favorite ? "Remover dos favoritos" : "Adicionar aos favoritos"} onClick={() => onFavorite(slug)}><Sparkles size={15} /></button>
  </div>;
}
...
export function ToolCatalog() {
  ...
  return <div className="reference-catalog">
    <div className="reference-catalog-head"><div><h1>Todas as ferramentas</h1><p>Escolha, favorite e retome rapidamente as ferramentas que você mais usa.</p></div><label className="reference-search"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar ferramenta..." /></label></div>
    {!normalizedQuery && active === "todas" && (favoriteTools.length || recentTools.length) ? <section className="reference-personal-tools" aria-label="Acesso rápido">
      {favoriteTools.length ? <div><header><span><Sparkles size={17} /></span><strong>Favoritas</strong><small>Ficam sempre à mão neste dispositivo.</small></header><div className="reference-personal-grid">{favoriteTools.slice(0, 6).map((tool) => <ToolItem key={tool.slug} tool={tool} favorite onFavorite={handleFavorite} />)}</div></div> : null}
      {recentTools.length ? <div><header><span><Repeat2 size={17} /></span><strong>Recentes</strong><small>Continue de onde parou.</small></header><div className="reference-personal-grid">{recentTools.map((tool) => <ToolItem key={tool.slug} tool={tool} favorite={false} onFavorite={handleFavorite} />)}</div></div> : null}
    </section> : null}
