...
import { ArrowRight, FileOutput, Files, Grid2X2, PencilLine, Repeat2, Search, ShieldCheck, Sparkles, X } from "lucide-react";
...
export function ToolCatalog({ initialQuery = "" }: { initialQuery?: string }) {
  ...
  const resultCount = filteredSections.reduce((total, group) => total + group.resolved.length, 0);
  const activeLabel = tabs.find((tab) => tab.id === active)?.label || "Todas";
  const hasFilters = Boolean(normalizedQuery) || active !== "todas";
  const clearFilters = () => {
    setQuery("");
    setActive("todas");
  };
  ...
  return <div className="reference-catalog">
    <div className="reference-catalog-head"><div><h1>Todas as ferramentas</h1><p>Escolha uma jornada. Cada função aparece uma só vez e os controles avançados ficam dentro do fluxo certo.</p></div><div className="catalog-filter"><label className="reference-search"><Search size={18} aria-hidden="true" /><input aria-label="Buscar no catálogo de ferramentas" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar dentro do catálogo..." />{query ? <button type="button" className="catalog-filter-clear" aria-label="Limpar busca do catálogo" onClick={() => setQuery("")}><X size={15} aria-hidden="true" /></button> : null}</label><span className="reference-search-note">Busca local no catálogo · aceita termos como imprimir, OCR, Word e juntar.</span></div></div>
    ...
    {!normalizedQuery && active === "todas" ? <Link className="catalog-print-feature" href="/ferramentas/criar-livreto-pdf"><span className="catalog-print-feature-icon"><FileOutput size={20} aria-hidden="true" /></span><span className="catalog-print-feature-copy"><strong>Preparar para impressão</strong><small>Monte livretos, use N-up, ajuste A4/A3/Carta e envie para a impressora do computador.</small></span><span className="catalog-print-feature-action">Abrir Centro de Impressão <ArrowRight size={15} aria-hidden="true" /></span></Link> : null}
    ...
    <div className="catalog-results-summary" role="status"><span><strong>{resultCount}</strong> {resultCount === 1 ? "ferramenta" : "ferramentas"}{normalizedQuery ? <> para <b>“{query.trim()}”</b></> : <> em <b>{activeLabel}</b></>}</span>{hasFilters ? <button type="button" onClick={clearFilters}>Limpar filtros <X size={14} aria-hidden="true" /></button> : null}</div>
